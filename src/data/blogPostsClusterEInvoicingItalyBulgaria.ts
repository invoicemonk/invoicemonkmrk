...
export const eInvoicingClusterPostsItalyBulgaria: BlogPost[] = [
  ...
  // ============================================================
  {
    ...
    content: `<p><strong>The Sistema di Interscambio (SdI) is the <a href="/en/e-invoicing/sdi-italy">Agenzia delle Entrate clearance hub</a> that validates and routes every Italian e-invoice. Sellers submit a FatturaPA XML; SdI runs schema and VAT-number checks, then forwards the invoice to the buyer using the 7-character Codice Destinatario or the buyer\'s PEC, returning a ricevuta di consegna on success or a ricevuta di scarto on rejection.</strong></p><div class="fact-box">
      ...
      <h2 id="what">What SdI is and is not</h2><p class="block-answer" data-answer="true">SdI is a clearance hub, not a national invoice archive in the GST-IRP sense.</p><p>SdI is a clearance hub, not a national invoice archive in the GST-IRP sense. It validates the FatturaPA against the AdE schema and schematron, registers the unique Identificativo SdI, and delivers the invoice. The legal original is the cleared XML itself, not a PDF rendering — the PDF (or HTML preview generated from the XML) is a courtesy view. SdI sits in the same architectural category as Türkiye\'s e-Fatura and ZATCA Phase 2: a pre-clearance gate without which the invoice has no legal existence between Italian-resident parties.</p><h2 id="flow">The five-step SdI flow</h2><p class="block-answer" data-answer="true">The five-step SdI flow includes: Seller\'s system builds a FatturaPA XML to schema v1.  2.</p><ol>
        <li>Seller\'s system builds a <a href="/en/glossary/fatturapa">FatturaPA</a> XML to schema v1.2.x with the correct TipoDocumento code.</li>
      ...
      </ol><h2 id="codice">Codice Destinatario routing</h2><p class="block-answer" data-answer="true">The 7-character alphanumeric Codice Destinatario tells SdI which buyer channel to use.  Italian VAT-registered buyers with an accredited intermediary register a code (e.</p><p>The 7-character alphanumeric <a href="/en/glossary/codice-destinatario">Codice Destinatario</a> tells SdI which buyer channel to use. Italian VAT-registered buyers with an accredited intermediary register a code (e.g. <code>KRRH6B9</code>). Buyers without one use <code>0000000</code> plus a registered PEC; private consumers without a PEC use <code>0000000</code> with the invoice deposited in their AdE personal area. Foreign buyers use <code>XXXXXXX</code>. An invalid code returns scarto 00311.</p><h2 id="td">TipoDocumento (TD) codes</h2><p class="block-answer" data-answer="true">FatturaPA encodes the invoice nature in a TD code: TD01 standard invoice, TD04 credit note, TD05 debit note, TD17 purchase of services from abroad (replaces Esterometro), TD18 intra-EU acquisition, TD19 art.</p><p>FatturaPA encodes the invoice nature in a TD code: TD01 standard invoice, TD04 credit note, TD05 debit note, TD17 purchase of services from abroad (replaces Esterometro), TD18 intra-EU acquisition, TD19 art.17 c.2 purchase, TD20 self-billing (autofattura), TD24 deferred invoice, TD28 purchases from San Marino. Picking the wrong TD is the most common semantic error and triggers AdE follow-up audits even when SdI accepts the document.</p><h2 id="errors">Common scarto codes</h2><p class="block-answer" data-answer="true">Common scarto codes includes: 00200 — file does not validate against the FatturaPA XSD.   00301 — CessionarioCommittente CodiceFiscale or PartitaIVA does not exist in AdE registry.</p><ul>
      ...
      </ul><h2 id="vida">What ViDA changes from 2030</h2><p class="block-answer" data-answer="true">The EU VAT in the Digital Age (ViDA) package introduces an EU-level digital reporting requirement based on EN 16931 / Peppol BIS Billing 3.</p><p>The EU VAT in the Digital Age (ViDA) package introduces an EU-level digital reporting requirement based on <a href="/en/glossary/peppol-bis-billing-3">EN 16931 / Peppol BIS Billing 3.0</a> for intra-EU transactions. AdE has signalled that SdI infrastructure will be retained — but the FatturaPA schema will need to expose the EN 16931 semantic model for intra-EU invoices alongside the current Italian-specific blocks. Domestic invoices remain on FatturaPA. Italy is one of the few Member States already operating clearance, so the operational change is mostly schema enrichment rather than a new channel.</p><h2>Key takeaways</h2><p class="block-answer" data-answer="true">Key takeaways includes: SdI is clearance, not archive: no Identificativo SdI = no legal invoice.   Codice Destinatario routing has three patterns: registered code, 0000000 + PEC, XXXXXXX for foreign.</p><ul>
      ...
      </ul><h2>Related reading</h2><p class="block-answer" data-answer="true">Related reading includes: How Invoicemonk implements SdI / FatturaPA How to comply with SdI: step-by-step</p><ul>
        <li><a href="/en/e-invoicing/sdi-italy">How Invoicemonk implements SdI / FatturaPA</a></li>
        <li><a href="/en/blog/how-to-comply-with-sdi-italy">How to comply with SdI: step-by-step</a></li>
        <li><a href="/en/blog/how-to-choose-e-invoicing-software-for-italian-vat-compliance">Choosing e-invoicing software for Italian VAT compliance</a></li>
        <li><a href="/en/blog/peppol-eu-explained">Peppol EU explained</a></li>
      </ul>`,
  ...
  // ============================================================
  {
    ...
    content: `<p><strong>To comply with SdI Italy, a VAT-registered business must obtain a partita IVA, register a Codice Destinatario (or use 0000000 plus PEC), build the <a href="/en/e-invoicing/sdi-italy">FatturaPA XML</a> with the correct TipoDocumento code, optionally sign with CAdES-BES, submit through SDICoop or an accredited intermediary, and reconcile the ricevuta di consegna against the issued invoice. The cleared XML is the legal original.</strong></p><div class="fact-box"><p><strong>At a glance</strong></p><ul>
      ...
      <h2 id="prereqs">Prerequisites</h2><p class="block-answer" data-answer="true">Prerequisites includes: Active Italian partita IVA (11 digits).   PEC mailbox (posta elettronica certificata) registered with INI-PEC.</p><ul>
        ...
        <li>Access to an <a href="/en/glossary/sdi">SdI</a>-capable invoicing system or accredited intermediary.</li>
      </ul><h2 id="onboarding">Onboarding: partita IVA, PEC, Codice Destinatario</h2><h3>Do I need to register a Codice Destinatario myself?</h3><p class="block-answer" data-answer="true">Only if you are a buyer who wants invoices routed to your invoicing system instead of your PEC.  Sellers do not need one.</p><p>Only if you are a buyer who wants invoices routed to your invoicing system instead of your PEC. Sellers do not need one. Most Italian businesses use their intermediary\'s Codice Destinatario, registered once in the AdE portal under "Registrazione dell\'indirizzo telematico". Once registered, the code overrides whatever the seller writes — SdI uses the registered code as the source of truth.</p><h3>Forfettari: do flat-rate taxpayers really have to e-invoice?</h3><p class="block-answer" data-answer="true">Yes, since 1 January 2024 (DL 36/2022 art. 18), regardless of turnover.</p><p>Yes, since 1 January 2024 (DL 36/2022 art.18), regardless of turnover. The previous €25k carve-out is gone. Most forfettari use accredited cloud platforms because the SDICoop SOAP integration is impractical for a single user.</p><h2 id="build">Build the FatturaPA XML</h2><p class="block-answer" data-answer="true">The XML root is &lt;FatturaElettronica&gt; with two top-level blocks:</p><p>The XML root is <code>&lt;FatturaElettronica&gt;</code> with two top-level blocks:</p><ul>
      ...
      </ul><h3>Picking the right TipoDocumento</h3><p>TD01 covers standard sales. TD04 credit note, TD05 debit note, TD20 self-billing (autofattura) for reverse-charge purchases, TD17/TD18/TD19 for cross-border. Invoicemonk picks the TD code from the counterparty\'s country and the VAT treatment, which removes the most common semantic error.</p><h2 id="submit">Submit to SdI</h2><p>Three live channels:</p><ol>
      ...
      </ol><p>SdI replies asynchronously with the ricevuta. Persist the Identificativo SdI on the invoice — it is the audit primary key.</p><h2 id="conservazione">Conservazione sostitutiva (10-year archive)</h2><p class="block-answer" data-answer="true">FatturaPA records must be preserved in conservazione sostitutiva (substitute electronic preservation) for 10 years per CAD art. 43 and DM 17 June 2014.</p><p>FatturaPA records must be preserved in <em>conservazione sostitutiva</em> (substitute electronic preservation) for 10 years per CAD art.43 and DM 17 June 2014. The conservation process applies a qualified timestamp and digital signature to a closed package of invoices, lodged with an AgID-accredited provider (or the AdE\'s own conservation service, free for small businesses).</p><h2 id="checklist">Go-live compliance checklist</h2><p class="block-answer" data-answer="true">Go-live compliance checklist includes: ☐ Partita IVA active and validated in VIES for intra-EU.   ☐ PEC registered with INI-PEC and the AdE portal.</p><ul>
      ...
      </ul><h2>Key takeaways</h2><p class="block-answer" data-answer="true">Key takeaways includes: SDICoop for volume, PEC for low volume, AdE portal for the smallest taxpayers.   Pick the TipoDocumento from the transaction, not from a default — TD code errors trigger audit selection.</p><ul>
      ...
      </ul><h2>Related reading</h2><p class="block-answer" data-answer="true">Related reading includes: SdI / FatturaPA explained Best e-invoicing software for small businesses in Italy</p><ul>
        <li><a href="/en/blog/sdi-italy-explained">SdI / FatturaPA explained</a></li>
        <li><a href="/en/blog/best-e-invoicing-software-for-small-businesses-in-italy">Best e-invoicing software for small businesses in Italy</a></li>
        <li><a href="/en/blog/how-to-choose-e-invoicing-software-for-italian-vat-compliance">How to choose for Italian VAT compliance</a></li>
      </ul>`,
    category: 'E-Invoicing',
  ...
  // ============================================================
  {
    ...
    content: `<p><strong>The best e-invoicing software for small businesses in Italy must do four things natively: build a valid <a href="/en/glossary/fatturapa">FatturaPA</a> XML, submit to the <a href="/en/e-invoicing/sdi-italy">Sistema di Interscambio</a> through SDICoop or PEC, route by Codice Destinatario, and lodge invoices in conservazione sostitutiva for 10 years. Invoicemonk, Fatture in Cloud, Aruba Fatturazione Elettronica, and FattureGo each meet that bar; the right pick depends on whether you need multi-jurisdiction coverage, accountant collaboration, or absolute price-floor.</strong></p><div class="fact-box"><p><strong>At a glance</strong></p><ul>
      ...
      <h2 id="criteria">The 7 criteria that matter</h2><p class="block-answer" data-answer="true">The 7 criteria that matter includes: SdI channel — SDICoop SOAP, PEC fallback, AdE portal upload.   The best platforms transparently fail over between them.</p><ol>
      ...
      </ol><h2 id="shortlist">The shortlist</h2><h3>1. Invoicemonk — best for cross-border Italian businesses</h3><p>Invoicemonk handles FatturaPA + SdI end to end with Codice Destinatario validation and conservazione sostitutiva to an AgID-accredited provider bundled. It is unique on this list in also covering 15 other mandates (ZATCA, Peppol, MyInvois, GST IRN, MTD, etc.), which matters if your buyers span Italy plus another regulated market. See the <a href="/en/e-invoicing/sdi-italy">SdI implementation page</a>.</p><h3>2. Fatture in Cloud (TeamSystem) — best for accountant collaboration</h3><p>Italian incumbent. Deep integration with Italian accountants (commercialisti). FatturaPA, SdI, conservazione, F24 — the full Italian compliance suite. Weak on cross-border and on UX for non-Italian-speaking founders.</p><h3>3. Aruba Fatturazione Elettronica — best price floor</h3><p>Bundled with Aruba PEC for low cost. Reliable SdI submission, conservazione included. Light on workflow and reporting.</p><h3>4. FattureGo — best pure-play SdI sender</h3><p>Free tier for very low volume. Limited features but does the SdI essentials.</p><h3>5. SumUp Fatture (ex-Tinaba) — best for service freelancers with payment links</h3><p>FatturaPA + payment link in one flow, suited to consultants who need to invoice and collect in the same step.</p><h2 id="matrix">Comparison matrix</h2><p class="block-answer" data-answer="true">PlatformSDICoopCodice Destinatario validationConservazione bundledForfettari planMulti-jurisdiction
        
          Invoicemonk✔✔ (AdE registry)✔ (AgID)✔18 mandates
          Fatture in Cloud✔✔✔✔Italy only
          Aruba✔Partial✔✔Italy only
          FattureGo✔✔Add-onFree tierItaly only
          SumUp…</p><div class="overflow-x-auto"><table>
      ...
      </table></div><h2 id="freelancer">Best for freelancers / forfettari</h2><p class="block-answer" data-answer="true">If you issue under 30 invoices a year, Fatture in Cloud\'s Forfettario plan or FattureGo\'s free tier are the lowest friction.  If you collect payments by card or link, SumUp Fatture compresses invoice + payment into one step.</p><p>If you issue under 30 invoices a year, Fatture in Cloud\'s Forfettario plan or FattureGo\'s free tier are the lowest friction. If you collect payments by card or link, SumUp Fatture compresses invoice + payment into one step. If you also have a foreign client base, Invoicemonk consolidates SdI and other mandates under one subscription.</p><h2 id="multicountry">Best if you invoice outside Italy too</h2><p class="block-answer" data-answer="true">This is where the list narrows.  Italian incumbents are domestic-only.</p><p>This is where the list narrows. Italian incumbents are domestic-only. <a href="/en/e-invoicing/peppol-eu">Peppol EU</a> coverage for German XRechnung, French Factur-X, or Belgian Hermes — plus non-EU regimes like Saudi <a href="/en/e-invoicing/zatca-phase-2">ZATCA Phase 2</a> or UK <a href="/en/e-invoicing/mtd-uk">MTD</a> — is where Invoicemonk\'s 16-mandate footprint pays for itself.</p><h2 id="mistakes">Three mistakes to avoid</h2><p class="block-answer" data-answer="true">Three mistakes to avoid includes: Picking a platform without conservazione bundled.   Buying conservation separately doubles the admin and often costs more.</p><ul>
      ...
      </ul><h2>Key takeaways</h2><p class="block-answer" data-answer="true">Key takeaways includes: The minimum viable feature set is SdI submission + Codice Destinatario validation + conservazione sostitutiva.   For cross-border invoicing, multi-mandate platforms win on TCO.</p><ul>
      ...
      </ul><h2>Related reading</h2><p class="block-answer" data-answer="true">Related reading includes: SdI / FatturaPA explained How to comply with SdI Italy</p><ul>
        <li><a href="/en/blog/sdi-italy-explained">SdI / FatturaPA explained</a></li>
        <li><a href="/en/blog/how-to-comply-with-sdi-italy">How to comply with SdI Italy</a></li>
        <li><a href="/en/blog/how-to-choose-e-invoicing-software-for-italian-vat-compliance">How to choose for Italian VAT compliance</a></li>
        <li><a href="/en/blog/enterprise-e-invoicing-europe-comparison">Enterprise e-invoicing in Europe — comparison</a></li>
      </ul>`,
    category: 'E-Invoicing',
  ...
  // ============================================================
  {
    ...
    content: `<p><strong>Choosing e-invoicing software for Italian VAT compliance means going beyond "does it talk to SdI?". The right platform proves it handles every <a href="/en/glossary/fatturapa">FatturaPA</a> TipoDocumento you issue, the Natura code matrix for VAT-exempt and reverse-charge supplies, IVA scissione dei pagamenti for public-administration buyers, and lodges every record in 10-year <em>conservazione sostitutiva</em>. Use the eight-point framework below.</strong></p><div class="fact-box"><p><strong>At a glance</strong></p><ul>
      ...
      <h2 id="framework">The 8-point framework</h2><p class="block-answer" data-answer="true">The 8-point framework includes: Native FatturaPA v1.  2.</p><ol>
      ...
      </ol><h2 id="td">TipoDocumento coverage — what to ask</h2><p class="block-answer" data-answer="true">"Show me how you handle TD17 (cross-border services purchase), TD20 (autofattura for missing supplier invoice), TD24 (deferred invoice), TD28 (San Marino purchases).</p><p>"Show me how you handle TD17 (cross-border services purchase), TD20 (autofattura for missing supplier invoice), TD24 (deferred invoice), TD28 (San Marino purchases)." If the demo cannot show these end to end, the platform is not safe for VAT compliance — defaulting to TD01 misclassifies the transaction and survives SdI validation but fails AdE risk-scoring.</p><h2 id="natura">Natura codes — the silent rejection trigger</h2><p class="block-answer" data-answer="true">Every line that does not charge standard 22% VAT needs a Natura code:</p><p>Every line that does not charge standard 22% VAT needs a Natura code:</p><ul>
      ...
      </ul><p>Missing or wrong Natura triggers scarto 00400 or 00415 — and even when accepted, an over-broad N2.2 is the single most common trigger of AdE letters of inquiry.</p><h2 id="scissione">Split payment and reverse-charge</h2><p class="block-answer" data-answer="true">Public administration buyers pay the supplier net and remit VAT directly to the AdE (scissione dei pagamenti, art.</p><p>Public administration buyers pay the supplier net and remit VAT directly to the AdE (scissione dei pagamenti, art.17-ter). The FatturaPA must carry <code>EsigibilitaIVA = S</code>. Reverse-charge (art.17 c.2/c.5/c.6) shifts VAT to the buyer with Natura N6.* and zero VAT charged. The platform must trigger the right combination from the buyer\'s nature (PA vs. private B2B) and the transaction code, not from a manual checkbox.</p><h2 id="conservazione">Conservazione sostitutiva — non-negotiable</h2><p class="block-answer" data-answer="true">10 years is the minimum.  The conservation pack must carry a qualified timestamp and digital signature, lodged with an AgID-accredited conservatore.</p><p>10 years is the minimum. The conservation pack must carry a qualified timestamp and digital signature, lodged with an AgID-accredited conservatore. The AdE\'s own free service is fine for low volume; for material volume, bundled conservazione from the e-invoicing vendor avoids running two compliance systems.</p><h2 id="vendor">Vendor due-diligence questions</h2><p class="block-answer" data-answer="true">Vendor due-diligence questions includes: "Which AgID-accredited conservatore do you use?  " "How do you handle FatturaPA schema upgrades — is there ever any downtime?</p><ul>
      ...
      </ul><h2>Key takeaways</h2><p class="block-answer" data-answer="true">Key takeaways includes: SdI submission is necessary but not sufficient — Italian VAT compliance lives in TipoDocumento, Natura, scissione, and conservazione.   Default-to-TD01 platforms survive SdI but fail audits.</p><ul>
      ...
      </ul><h2>Related reading</h2><p class="block-answer" data-answer="true">Related reading includes: SdI / FatturaPA explained Best SdI-ready platforms for SMBs</p><ul>
        <li><a href="/en/blog/sdi-italy-explained">SdI / FatturaPA explained</a></li>
        <li><a href="/en/blog/best-e-invoicing-software-for-small-businesses-in-italy">Best SdI-ready platforms for SMBs</a></li>
        <li><a href="/en/blog/choosing-an-e-invoicing-platform-checklist">12-point multi-jurisdiction checklist</a></li>
      </ul>`,
    category: 'E-Invoicing',
  ...
  // ============================================================
  {
    ...
    content: `<p><strong>Bulgaria operates two distinct compliance surfaces. <a href="/en/glossary/saf-t-bulgaria">SAF-T BG</a> — Bulgaria\'s adoption of the OECD SAF-T 2.0 schema with NRA extensions — rolls out by enterprise size from 1 January 2026 (large) to 1 January 2030 (micro). B2G invoices have travelled through <a href="/en/glossary/peppol-bis-billing-3">Peppol BIS Billing 3.0</a> since 2019. A domestic B2B clearance mandate is in public consultation. See the <a href="/en/e-invoicing/nra-bulgaria">NRA mandate page</a> for Invoicemonk\'s implementation.</strong></p><div class="fact-box"><p><strong>At a glance</strong></p><ul>
      ...
      <h2 id="saft">What SAF-T BG covers</h2><p class="block-answer" data-answer="true">SAF-T BG is an XML file the NRA can request (and, on the wave dates, that taxpayers must submit on a recurring cadence).  The schema follows OECD SAF-T 2.</p><p>SAF-T BG is an XML file the NRA can request (and, on the wave dates, that taxpayers must submit on a recurring cadence). The schema follows OECD SAF-T 2.0 with Bulgarian-specific code lists and extensions, structured as:</p><ul>
      ...
      </ul><h2 id="waves">Rollout waves</h2><p class="block-answer" data-answer="true">The NRA defines four bands by turnover, assets, and headcount under the Bulgarian Accountancy Act classification, plus a "group 1 / very large" tier:</p><p>The NRA defines four bands by turnover, assets, and headcount under the Bulgarian Accountancy Act classification, plus a "group 1 / very large" tier:</p><ul>
      ...
      </ul><p>Submission is via the NRA SAF-T portal with a qualified electronic signature (QES) on the file envelope.</p><h2 id="peppol">B2G via Peppol</h2><p class="block-answer" data-answer="true">Since November 2019, supplying Bulgarian public bodies requires a Peppol BIS Billing 3.</p><p>Since November 2019, supplying Bulgarian public bodies requires a Peppol BIS Billing 3.0 UBL invoice routed through a <a href="/en/glossary/peppol-access-point">certified Access Point</a>. Public bodies register under scheme <code>9938:VAT-number</code>. This transposes EU Directive 2014/55/EU into Bulgarian law via amendments to the Public Procurement Act (art.39a). Routing is end-to-end UBL; the public body\'s ERP receives the structured invoice directly.</p><h2 id="consultation">Domestic B2B consultation</h2><p class="block-answer" data-answer="true">The Ministry of Finance published a domestic e-invoicing consultation paper in late 2025.</p><p>The Ministry of Finance published a domestic e-invoicing consultation paper in late 2025. The current direction is a clearance-style continuous-transaction-controls regime (similar to Italy\'s SdI), with the NRA operating the hub. The expected timeline is large-enterprise B2B alongside the small-enterprise SAF-T wave in 2028, with full rollout by 2030. The schema is expected to align with EN 16931 to satisfy <a href="/en/blog/peppol-eu-explained">ViDA</a>.</p><h2 id="zdds">How SAF-T relates to ZDDS VAT returns</h2><p class="block-answer" data-answer="true">SAF-T does not replace the monthly VAT return (декларация по ЗДДС) or the VIES recapitulative statement.  It provides the NRA with the underlying transactional dataset so risk-scoring and audit selection can run against the source data rather than aggregated returns.</p><p>SAF-T does <em>not</em> replace the monthly VAT return (декларация по ЗДДС) or the VIES recapitulative statement. It provides the NRA with the underlying transactional dataset so risk-scoring and audit selection can run against the source data rather than aggregated returns. Expect post-rollout audits to start with cross-checks between the SAF-T SourceDocuments and the corresponding VAT return lines.</p><h2>Key takeaways</h2><p class="block-answer" data-answer="true">Key takeaways includes: SAF-T BG is the dominant Bulgarian compliance surface from 2026; B2G Peppol has been live since 2019.   Wave date is set by enterprise size — large first, micro last.</p><ul>
      ...
      </ul><h2>Related reading</h2><p class="block-answer" data-answer="true">Related reading includes: How Invoicemonk implements NRA Bulgaria Best e-invoicing platforms for Bulgarian SMBs</p><ul>
        <li><a href="/en/e-invoicing/nra-bulgaria">How Invoicemonk implements NRA Bulgaria</a></li>
        <li><a href="/en/blog/best-e-invoicing-platforms-for-small-businesses-in-bulgaria">Best e-invoicing platforms for Bulgarian SMBs</a></li>
        <li><a href="/en/blog/how-to-choose-e-invoicing-provider-bulgarian-tax-laws">Choosing a Bulgarian-tax-compliant provider</a></li>
        <li><a href="/en/blog/peppol-eu-explained">Peppol EU explained</a></li>
      </ul>`,
    category: 'E-Invoicing',
  ...
  // ============================================================
  {
    ...
    content: `<p><strong>The best e-invoicing platforms for Bulgarian small businesses already handle ZDDS-compliant invoices today, generate <a href="/en/glossary/saf-t-bulgaria">SAF-T BG</a> for the 2026–2030 wave that applies to you, and route B2G invoices as <a href="/en/glossary/peppol-bis-billing-3">Peppol BIS Billing 3.0</a>. Invoicemonk, Microinvest, Balans, and Plus Minus are the shortlist; the right pick depends on whether you need cross-border coverage, deep Bulgarian accounting integration, or a low-cost ZDDS-only invoicer.</strong></p><div class="fact-box"><p><strong>At a glance</strong></p><ul>
      ...
      <h2 id="criteria">The 6 criteria that matter</h2><p class="block-answer" data-answer="true">The 6 criteria that matter includes: ZDDS-compliant invoice fields — Bulgarian VAT number format (BG + 9–10 digits), reverse-charge under art.  82 / art.</p><ol>
      ...
      </ol><h2 id="shortlist">The shortlist</h2><h3>1. Invoicemonk — best for Bulgarian businesses with cross-border buyers</h3><p>Invoicemonk issues ZDDS-compliant invoices, generates SAF-T BG XML for the relevant wave, and routes Peppol BIS Billing 3.0 to Bulgarian public bodies via a certified Access Point. Multi-currency with euro changeover handling is built in. The 16-mandate footprint (ZATCA, Peppol, MTD, MyInvois, etc.) matters for Bulgarian exporters. See the <a href="/en/e-invoicing/nra-bulgaria">NRA mandate page</a>.</p><h3>2. Microinvest Delta Pro — best for deep Bulgarian accounting integration</h3><p>Bulgarian incumbent. Tightly integrated with Bulgarian accountants, fiscal device support, full ZDDS toolkit. SAF-T BG roadmap announced; Peppol via partner. Weak on UI and cross-border.</p><h3>3. Balans — best for accountant-facing workflow</h3><p>Mid-market Bulgarian platform used by many счетоводни кантори. Strong ZDDS reporting, monthly close, SAF-T modules in beta.</p><h3>4. Plus Minus — best price floor for ZDDS-only invoicing</h3><p>Long-running Bulgarian invoicing tool. Affordable, ZDDS-correct, modest on automation. SAF-T BG generation is on the roadmap rather than shipped.</p><h3>5. Invoicemonk competitor benchmark — Smart Bulgaria</h3><p>Smart Bulgaria is gaining traction with start-ups for its API-first model; SAF-T BG is on roadmap, Peppol via partner.</p><h2 id="matrix">Comparison matrix</h2><p class="block-answer" data-answer="true">PlatformZDDS invoiceSAF-T BG readyPeppol B2GQES signingMulti-jurisdiction
        
          Invoicemonk✔✔ (all waves)✔ (certified AP)✔18 mandates
          Microinvest Delta Pro✔RoadmapVia partner✔BG only
          Balans✔BetaVia partner✔BG only
          Plus Minus✔Roadmap—PartialBG only
          Smart…</p><div class="overflow-x-auto"><table>
      ...
      </table></div><h2 id="freelancer">Best for freelancers</h2><p class="block-answer" data-answer="true">Single-person businesses below the SAF-T wave threshold (until 2030) should optimise for ZDDS correctness and low subscription.  Plus Minus or Microinvest\'s freelancer tier suffice for purely domestic invoicing.</p><p>Single-person businesses below the SAF-T wave threshold (until 2030) should optimise for ZDDS correctness and low subscription. Plus Minus or Microinvest\'s freelancer tier suffice for purely domestic invoicing. If you have EU buyers, Invoicemonk\'s Peppol output and FatturaPA / XRechnung handling pays for itself when those buyers ask for structured invoices.</p><h2 id="future">Picking for the 2027–2030 wave</h2><p class="block-answer" data-answer="true">If your wave is years away, the temptation is to defer the choice.  The risk is that ZDDS-only platforms reach the wave with rushed SAF-T modules.</p><p>If your wave is years away, the temptation is to defer the choice. The risk is that ZDDS-only platforms reach the wave with rushed SAF-T modules. Vendor commitment to SAF-T BG (and the upcoming CTC clearance) should be a contract-level commitment, not a brochure claim. Ask for a roadmap, a beta sandbox, and the named AgID-equivalent (BG QES TSP) used.</p><h2>Key takeaways</h2><p class="block-answer" data-answer="true">Key takeaways includes: Today\'s pick must cover ZDDS + Peppol B2G; tomorrow\'s pick must cover SAF-T BG and the planned domestic CTC.   Multi-jurisdiction matters for Bulgarian exporters because EU buyers increasingly require Peppol BIS Billing 3.</p><ul>
      ...
      </ul><h2>Related reading</h2><p class="block-answer" data-answer="true">Related reading includes: NRA Bulgaria explained Choosing a provider compliant with Bulgarian tax laws</p><ul>
        <li><a href="/en/blog/nra-bulgaria-explained">NRA Bulgaria explained</a></li>
        <li><a href="/en/blog/how-to-choose-e-invoicing-provider-bulgarian-tax-laws">Choosing a provider compliant with Bulgarian tax laws</a></li>
        <li><a href="/en/blog/enterprise-e-invoicing-europe-comparison">Enterprise e-invoicing in Europe — comparison</a></li>
      </ul>`,
    category: 'E-Invoicing',
  ...
  // ============================================================
  {
    ...
    content: `<p><strong>Choosing an e-invoicing provider compliant with Bulgarian tax laws means proving the vendor handles four current obligations (<a href="/en/e-invoicing/nra-bulgaria">ZDDS VAT invoice, Accountancy Act records, B2G Peppol, QES signing</a>) and two upcoming ones (SAF-T BG on your wave, the planned NRA clearance regime). Use the framework below as a vendor-RFP scoring grid.</strong></p><div class="fact-box"><p><strong>At a glance</strong></p><ul>
      ...
      <h2 id="framework">The 8-criterion framework</h2><p class="block-answer" data-answer="true">The 8-criterion framework includes: ZDDS-compliant invoice field set (BG VAT, 20%/9%/0%, reverse-charge under art.  82 / art.</p><ol>
      ...
      </ol><h2 id="zdds">ZDDS field coverage</h2><p class="block-answer" data-answer="true">ZDDS art. 114 mandates: sequential invoice number, date, supplier and recipient details (incl.</p><p>ZDDS art.114 mandates: sequential invoice number, date, supplier and recipient details (incl. BG VAT identification number), description, quantity, unit price, tax base, VAT rate and amount, total. Reverse-charge invoices must carry the explicit "обратно начисляване" annotation plus the legal basis (art.82 c.2 / art.163a). Triangulation under art.65a must be flagged. Verify the vendor renders all of this on the standard PDF and in the SAF-T SourceDocuments block.</p><h2 id="accountancy">Accountancy Act retention</h2><p class="block-answer" data-answer="true">Bulgarian Accountancy Act (Закон за счетоводството) art. 12 requires 10-year retention of primary accounting documents, including invoices.</p><p>Bulgarian Accountancy Act (Закон за счетоводството) art.12 requires 10-year retention of primary accounting documents, including invoices. The archive must be immutable and addressable per document. Look for object-lock or WORM-equivalent guarantees, not just "we keep backups".</p><h2 id="saft">SAF-T BG readiness</h2><p class="block-answer" data-answer="true">The NRA published the SAF-T BG technical specification with the OECD SAF-T 2. 0 base schema plus Bulgarian code lists.</p><p>The NRA published the SAF-T BG technical specification with the OECD SAF-T 2.0 base schema plus Bulgarian code lists. Verify the provider:</p><ul>
      ...
      </ul><h2 id="peppol">Peppol B2G</h2><p class="block-answer" data-answer="true">Public Procurement Act art. 39a mandates Peppol for tenders.</p><p>Public Procurement Act art.39a mandates Peppol for tenders. The provider must hold (or partner with) a certified Peppol Access Point and register your business under scheme <code>9938:VAT-number</code>. Verify the provider can show buyer-side acceptance from at least one major Bulgarian public buyer.</p><h2 id="ctc">Future CTC clearance</h2><p class="block-answer" data-answer="true">The Ministry of Finance consultation paper points to a clearance-style regime on the Italian SdI model.</p><p>The Ministry of Finance consultation paper points to a clearance-style regime on the Italian SdI model. The provider\'s roadmap should commit to:</p><ul>
      ...
      </ul><h2 id="rfp">Vendor RFP questions</h2><p class="block-answer" data-answer="true">Vendor RFP questions includes: "Show me an end-to-end ZDDS reverse-charge invoice under art.  163a in your demo.</p><ul>
      ...
      </ul><h2>Key takeaways</h2><p class="block-answer" data-answer="true">Key takeaways includes: Compliance is multi-surface: ZDDS invoices today, SAF-T tomorrow, CTC the day after.   Treat the SAF-T and CTC roadmap as contract terms, not marketing.</p><ul>
        ...
        <li><a href="https://www.minfin.bg/" rel="noopener noreferrer">Ministry of Finance</a></li>
      </ul><h2>Related reading</h2><p class="block-answer" data-answer="true">Related reading includes: NRA Bulgaria explained Best platforms for Bulgarian SMBs</p><ul>
        <li><a href="/en/blog/nra-bulgaria-explained">NRA Bulgaria explained</a></li>
        <li><a href="/en/blog/best-e-invoicing-platforms-for-small-businesses-in-bulgaria">Best platforms for Bulgarian SMBs</a></li>
        <li><a href="/en/blog/choosing-an-e-invoicing-platform-checklist">12-point multi-jurisdiction checklist</a></li>
      </ul>`,
    category: 'E-Invoicing',
  ...
  // ============================================================
  {
    ...
    content: `<p><strong>Enterprise e-invoicing in Europe is shaped by three forces: 27 national mandates at different maturities, the EU-wide <a href="/en/blog/peppol-eu-explained">Peppol BIS Billing 3.0</a> backbone, and the ViDA digital reporting requirement from 2030. The best enterprise platforms cover the live mandates (Italy <a href="/en/e-invoicing/sdi-italy">SdI</a>, France PPF/PDP, Spain TicketBAI/Veri*Factu, Germany B2B 2025–2028, Poland KSeF, Romania e-Factura, plus Bulgaria <a href="/en/e-invoicing/nra-bulgaria">SAF-T</a> and the rest), integrate natively with SAP S/4HANA, Oracle Fusion, Microsoft Dynamics 365, NetSuite, and Sage X3, and ship ViDA-ready EN 16931 schemas today.</strong></p><div class="fact-box"><p><strong>At a glance</strong></p><ul>
      ...
      <h2 id="landscape">The European mandate landscape</h2><p class="block-answer" data-answer="true">Europe is the most fragmented e-invoicing market in the world.</p><p>Europe is the most fragmented e-invoicing market in the world. Italy has been on full B2B clearance since 2019; France launched PPF/PDP in stages 2024–2026; Spain runs TicketBAI in the Basque Country and Veri*Factu nationally; Germany\'s B2B mandate phases in 2025–2028; Poland\'s KSeF was deferred to 2026; Romania\'s e-Factura is live for B2B since 2024; Greece\'s myDATA is the real-time reporting baseline. Bulgaria\'s SAF-T phases in 2026–2030. ViDA layers an EU-wide EN 16931 reporting obligation on top from 2030 for intra-EU transactions.</p><h2 id="vendors">The enterprise shortlist</h2><h3>1. Invoicemonk — best for multi-mandate breadth without per-country add-ons</h3><p>Native coverage of 18 mandates including SdI, Peppol EU, MTD, plus the upcoming Bulgarian and German B2B regimes. One subscription, one schema engine. Integrates with SAP, Oracle Fusion, Dynamics 365, NetSuite, Sage X3, Odoo, QuickBooks, and Xero through standard connectors.</p><h3>2. SAP Document and Reporting Compliance — best if you are SAP-mandated</h3><p>The native SAP option. Strong on SAP ERP integration, weaker on non-SAP back-ends. Per-country activation pricing.</p><h3>3. Pagero — best for Peppol-heavy Nordic operations</h3><p>Long-running European e-invoicing operator now part of Thomson Reuters. Strong Peppol presence, broad EU coverage, ERP-agnostic.</p><h3>4. Sovos — best for indirect tax integration</h3><p>Tax-compliance vendor that bundles e-invoicing with VAT determination. Strong in jurisdictions with complex tax-rate logic (ES, IT, PT).</p><h3>5. Edicom — best for Spain and LatAm bridge</h3><p>Spain-headquartered; deep TicketBAI and Veri*Factu coverage; LatAm coverage for businesses spanning ES + MX/BR/CL.</p><h3>6. Comarch EDI — best for CEE depth</h3><p>Poland-headquartered; strong KSeF, RO e-Factura, HU RTIR; broad EDI catalogue alongside e-invoicing.</p><h2 id="matrix">Coverage and ERP integration matrix</h2><p class="block-answer" data-answer="true">PlatformIT SdIFR PPFES TicketBAIDE B2BPL KSeFBG SAF-TSAP S/4Oracle FusionD365 F&amp;ONetSuiteSage X3
        
          Invoicemonk✔✔✔✔✔✔✔✔✔✔✔
          SAP DRC✔✔✔✔✔PartialNative✗✗✗✗
          Pagero✔✔✔✔✔Partial✔✔✔✔✔
          Sovos✔✔✔✔✔Partial✔✔✔✔Partial
          Edicom✔✔✔ (deep)✔✔Partial✔✔✔✔Partial…</p><div class="overflow-x-auto"><table>
      ...
      </table></div><h2 id="integration">Integration capabilities with existing accounting systems</h2><p class="block-answer" data-answer="true">Enterprise selection often comes down to how cleanly the platform sits in front of (or beside) the accounting system.</p><p>Enterprise selection often comes down to how cleanly the platform sits in front of (or beside) the accounting system. The mature pattern is:</p><ul>
      ...
      </ul><p>The integration questions that matter at enterprise scale: bidirectional customer / supplier sync, real-time webhook on clearance status, replay of failed transmissions, correlation IDs persisted on both sides, and a tested SAP-mandate IDoc upgrade path. Brochure-grade "integrates with SAP" claims should be tested with a 30-day sandbox.</p><h2 id="vida">ViDA readiness</h2><p class="block-answer" data-answer="true">From 2030 (or earlier, depending on Council adoption of revised timelines), intra-EU B2B transactions must be reported as structured EN 16931 messages within near-real-time of issuance.</p><p>From 2030 (or earlier, depending on Council adoption of revised timelines), intra-EU B2B transactions must be reported as structured EN 16931 messages within near-real-time of issuance. Enterprises with cross-border EU activity should confirm:</p><ul>
      ...
      </ul><h2 id="choose">How enterprises should choose</h2><p class="block-answer" data-answer="true">Map your live ERPs to the vendor matrix.  Eliminate vendors lacking native connectors for your top three.</p><ol>
      ...
      </ol><h2>Key takeaways</h2><p class="block-answer" data-answer="true">Key takeaways includes: European e-invoicing is irreducibly multi-jurisdiction; single-country tools are dead-ends at enterprise scale.   Native ERP integration with the top platforms (SAP, Oracle, Dynamics, NetSuite, Sage) is the second decisive axis.</p><ul>
      ...
      </ul><h2>Related reading</h2><p class="block-answer" data-answer="true">Related reading includes: E-invoicing pillar — 18 mandates Peppol EU explained</p><ul>
        <li><a href="/en/e-invoicing">E-invoicing pillar — 18 mandates</a></li>
        <li><a href="/en/blog/peppol-eu-explained">Peppol EU explained</a></li>
        <li><a href="/en/blog/sdi-italy-explained">SdI / FatturaPA explained</a></li>
        <li><a href="/en/blog/nra-bulgaria-explained">NRA Bulgaria explained</a></li>
        <li><a href="/en/blog/choosing-an-e-invoicing-platform-checklist">12-point buyer\'s checklist</a></li>
      </ul>`,
    category: 'E-Invoicing',
