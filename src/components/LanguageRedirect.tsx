...
// Timezone → country mapping (same as before)
 ...
 * and redirects to the appropriate language prefix (e.g. `/en/`, `/de/`).
 */
  ...
  useEffect(() => {
    const redirect = async () => {
      ...
      if (saved && supportedCountries.includes(saved as SupportedCountry)) {
        const lang = getLanguageForCountry(saved as SupportedCountry);
      ...
      if (tzCountry) {
        ...
        const lang = getLanguageForCountry(tzCountry);
        navigate(`/${lang}/`, { replace: true });
        return;
      }
      ...
      if (ipCountry) {
        ...
        const lang = getLanguageForCountry(ipCountry);
        navigate(`/${lang}/`, { replace: true });
        return;
      }
