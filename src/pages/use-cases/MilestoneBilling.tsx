...
const stepIcons = [FileText, Target, TrendingUp];
const featureIcons = [FileText, Eye, DollarSign, Shield, Clock, Settings];

export default function MilestoneBilling() {
  const { t } = useTranslation('milestoneBilling');
  ...
  const idealItems = t('idealFor.items', { returnObjects: true }) as string[];
  ...
  return (
    <Layout>
      ...
      <BreadcrumbSchema items={[{ name: 'Home', url: '/' }, { name: 'Use Cases', url: '/milestone-billing' }, { name: 'Milestone Billing', url: '/milestone-billing' }]} />
      <PageHero
        badge={{ icon: Target, text: t('hero.badge') }}
        title={t('hero.title')}
      ...
      <section className="py-16 lg:py-24">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl">
          <h2 className="text-heading-lg font-bold text-foreground text-center mb-4">{t('idealFor.title')}</h2>
          <p className="text-muted-foreground text-center max-w-2xl mx-auto mb-8">{t('idealFor.subtitle')}</p>
